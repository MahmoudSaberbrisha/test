<div>
    ...
    @if ($modalData)
        <form wire:loading.remove action="{{route(auth()->getDefaultDriver().'.extra-services.update', $modalData->id)}}" method="POST" enctype="multipart/form-data" class="advanced-form myForm" id="my-form-id" data-parsley-validate x-data x-init="
                        ...
                        <div class="form-group">
                            ...
                            <select name="parent_id" id="parent_id" class="form-control">
                                ...
                                @foreach ($parents as $parent)
                                    <option value="{{$parent->id}}" {{$modalData->parent_id==$parent->id?'selected':''}}>{{$parent->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
