...
@section('content')
    ...
    <form action="{{ route('storehadbackpurchase.update', $hadback->id) }}" method="POST" style="color: white;">
        @csrf
        @method('PUT')
        <div class="row">
                ...
                <select class="form-select form-control-3d" id="main_branch_id_fk" name="main_branch_id_fk" required
                    ...
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->id }}"
                            {{ old('main_branch_id_fk', $hadback->main_branch_id_fk) == $branch->id ? 'selected' : '' }}>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select class="form-select form-control-3d" id="sub_branch_id_fk" name="sub_branch_id_fk" required
                    ...
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->id }}"
                            {{ old('sub_branch_id_fk', $hadback->sub_branch_id_fk) == $branch->id ? 'selected' : '' }}>
                            {{ $branch->title ?? $branch->id }}
                        </option>
                    @endforeach
                </select>
                ...
                <select class="form-select form-control-3d" id="supplier_code" name="supplier_code" required
                    ...
                    @foreach ($suppliers as $supplier)
                        <option value="{{ $supplier->code }}"
                            {{ old('supplier_code', $hadback->supplier_code) == $supplier->code ? 'selected' : '' }}>
                ...
                <select class="form-select form-control-3d" id="fatora_code" name="fatora_code" required
                    ...
                    @foreach ($invoices as $invoice)
                        <option value="{{ $invoice->fatora_code }}" data-amount-buy="{{ $invoice->amount_buy }}"
                            ...
                            {{ old('fatora_code', $hadback->fatora_code) == $invoice->fatora_code ? 'selected' : '' }}>
                            {{ $invoice->fatora_code }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control form-control-3d" id="product_name" name="product_name" readonly
                    value="{{ old('product_name', $hadback->product_name) }}" style="color: black;">
            </div>
            <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control form-control-3d" id="product_code" name="product_code" required
                    value="{{ old('product_code', $hadback->product_code) }}" style="color: black;">
            </div>
            <div class="col-md-6 mb-3">
                ...
                <input type="number" step="0.01" class="form-control form-control-3d" id="amount_buy" name="amount_buy"
                    required value="{{ old('amount_buy', $hadback->amount_buy) }}" style="color: black;">
            </div>
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="number" step="0.01" class="form-control form-control-3d" id="all_cost_buy"
                    name="all_cost_buy" required value="{{ old('all_cost_buy', $hadback->all_cost_buy) }}"
                    style="color: black;">
            </div>
            <div class="col-md-6 mb-3">
                ...
                <input type="number" step="0.01" class="form-control form-control-3d" id="one_price_sell"
                    name="one_price_sell" required value="{{ old('one_price_sell', $hadback->one_price_sell) }}"
                    style="color: black;">
            </div>
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="number" class="form-control form-control-3d" id="hadback_amount" name="hadback_amount"
                    required value="{{ old('hadback_amount', $hadback->hadback_amount) }}" style="color: black;">
            </div>
            <div class="col-md-6 mb-3">
                <label for="date" class="form-label">التاريخ </label>
                <input type="date" class="form-control form-control-3d" id="date" name="date" required
                    value="{{ old('date', $hadback->date) }}" style="color: black;">
            </div>
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="date" class="form-control form-control-3d" id="date_s" name="date_s" required
                    value="{{ old('date_s', $hadback->date_s) }}" style="color: black;">
            </div>
            <div class="col-md-6 mb-3">
                ...
                <select class="form-select form-control-3d" id="publisher" name="publisher" required
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}"
                            {{ old('publisher', $hadback->publisher) == $user->id ? 'selected' : '' }}>
                            {{ $user->name ?? $user->id }}
                        </option>
                    @endforeach
                </select>
        ...
        <button type="submit" class="btn btn-primary">تحديث عملية شراء مرتجعة</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const onePriceSellInput = document.getElementById('one_price_sell');
            ...
            fatoraSelect.addEventListener('change', function() {
                ...
                if (onePriceSell) {
                    onePriceSellInput.value = onePriceSell;
                } else {
                    onePriceSellInput.value = '';
                }
            });
        });
    </script>
