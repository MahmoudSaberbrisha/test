...
@section('content')
    ...
    <form action="{{ route('admin.storekhazina.update', $khazina->id) }}" method="POST" style="color: white;">
        @csrf
        @method('PUT')
        <div class="row">
                ...
                <select class="form-select form-control-3d" id="main_branch_id_fk" name="main_branch_id_fk" required>
                    ...
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->id }}"
                            {{ old('main_branch_id_fk', $khazina->main_branch_id_fk) == $branch->id ? 'selected' : '' }}>
                            {{ $branch->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select class="form-select form-control-3d" id="sub_branch_id_fk" name="sub_branch_id_fk" required>
                    ...
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->id }}"
                            {{ old('sub_branch_id_fk', $khazina->sub_branch_id_fk) == $branch->id ? 'selected' : '' }}>
                            {{ $branch->title }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="row">
            <div class="col-md-12 mb-3">
                ...
                <input type="text" class="form-control form-control-3d" id="name" name="name" required
                    maxlength="255" value="{{ old('name', $khazina->name) }}">
            </div>
        </div>
        <div class="row">
            <div class="col-md-12 mb-3">
                <label for="balance" class="form-label">الرصيد</label>
                <input type="number" step="0.01" min="0" class="form-control form-control-3d" id="balance"
                    name="balance" required value="{{ old('balance', $khazina->balance) }}">
            </div>
        </div>
