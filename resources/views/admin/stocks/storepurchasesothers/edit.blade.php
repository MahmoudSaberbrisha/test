...
@section('content')
    <h1>تعديل فاتورة مشتريات أخرى</h1>
    ...
    <form action="{{ route('admin.storepurchasesothers.update', $purchase->id) }}" method="POST"
        class="p-4 border rounded shadow-sm" style="color: white;">
        @csrf
        @method('PUT')
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <select class="form-select form-control-3d" id="main_branch_id_fk" name="main_branch_id_fk" required>
                    ...
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->id }}"
                            {{ old('main_branch_id_fk', $purchase->main_branch_id_fk) == $branch->id ? 'selected' : '' }}>
            ...
            <div class="col-md-6">
                ...
                <select class="form-select form-control-3d" id="sub_branch_id_fk" name="sub_branch_id_fk" required>
                    ...
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->id }}"
                            {{ old('sub_branch_id_fk', $purchase->sub_branch_id_fk) == $branch->id ? 'selected' : '' }}>
                            {{ $branch->title ?? $branch->id }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <input type="text" class="form-control form-control-3d" id="product_code" name="product_code" required
                    value="{{ old('product_code', $purchase->product_code) }}" readonly>
            </div>
            <div class="col-md-4">
                ...
                <select class="form-select form-control-3d" id="product_name" name="product_name" required>
                    ...
                    @foreach ($products as $product)
                        <option value="{{ $product->name }}" data-product-code="{{ $product->sanf_code }}"
                            {{ old('product_name', $purchase->product_name) == $product->name ? 'selected' : '' }}>
                            {{ $product->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <input type="number" step="0.01" class="form-control form-control-3d" id="amount_buy" name="amount_buy"
                    required value="{{ old('amount_buy', $purchase->amount_buy) }}">
            </div>
            <div class="col-md-4">
                ...
                <input type="text" class="form-control form-control-3d" id="all_cost_buy" name="all_cost_buy" required
                    value="{{ old('all_cost_buy', $purchase->all_cost_buy) }}" readonly>
            </div>
            <div class="col-md-4">
                ...
                <input type="number" step="0.01" class="form-control form-control-3d" id="one_price_sell"
                    name="one_price_sell" required value="{{ old('one_price_sell', $purchase->one_price_sell) }}">
            </div>
            <div class="col-md-4">
                ...
                <input type="number" step="0.01" class="form-control form-control-3d" id="one_price_buy"
                    name="one_price_buy" required value="{{ old('one_price_buy', $purchase->one_price_buy) }}">
            </div>
            <div class="col-md-4">
                ...
                <input type="number" step="0.01" class="form-control form-control-3d" id="rasid_motah"
                    name="rasid_motah" required value="{{ old('rasid_motah', $purchase->rasid_motah) }}">
            </div>
            <div class="col-md-4">
                ...
                <select class="form-select form-control-3d" id="publisher" name="publisher" required>
                    ...
                    @foreach ($publishers as $publisher)
                        <option value="{{ $publisher->id }}"
                            {{ old('publisher', $purchase->publisher) == $publisher->id ? 'selected' : '' }}>
            ...
            <div class="col-md-4">
                ...
                <input type="number" step="0.01" class="form-control form-control-3d" id="had_back"
                    name="had_back" required value="{{ old('had_back', $purchase->had_back) }}">
            </div>
            <div class="col-md-4">
                ...
                <input type="number" step="0.01" class="form-control form-control-3d" id="had_back_amount"
                    name="had_back_amount" required value="{{ old('had_back_amount', $purchase->had_back_amount) }}">
            </div>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                ...
                <input type="date" class="form-control form-control-3d" id="fatora_print_date"
                    name="fatora_print_date" value="{{ old('fatora_print_date', $purchase->fatora_print_date) }}">
            </div>
        </div>
                ...
                <select class="form-select form-control-3d" id="supplier_code" name="supplier_code" required>
                    ...
                    @foreach ($suppliers as $supplier)
                        <option value="{{ $supplier->code }}"
                            {{ old('supplier_code', $purchase->supplier_code) == $supplier->code ? 'selected' : '' }}>
        ...
        <div class="row g-3 mt-3">
                ...
                <select class="form-select form-control-3d" id="box_name" name="box_name" onchange="updateBoxId()">
                    ...
                    @foreach ($boxes as $box)
                        <option value="{{ $box->id }}" data-box-id="{{ $box->id }}"
                            {{ old('box_name', $purchase->box_name) == $box->id ? 'selected' : '' }}>
                            {{ $box->name ?? $box->id }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-6">
                ...
                <input type="number" class="form-control form-control-3d" id="box_id_fk" name="box_id_fk"
                    value="{{ old('box_id_fk', $purchase->box_id_fk) }}" readonly>
            </div>
        </div>
        ...
        <div class="mt-4 text-center">
            <button type="submit" class="btn btn-primary">تحديث فاتورة مشتريات أخرى</button>
