...
@section('content')
    ...
    <form action="{{ route('admin.storemasrofasnaffar3.update', $record->id) }}" method="POST" style="color: white;">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="main_branch_fk" class="form-label">الفرع الرئيسي</label>
                <select class="form-select form-control-3d" id="main_branch_fk" name="main_branch_fk" required>
                    <option value="">اختر الفرع الرئيسي</option>
                    @foreach ($mainBranches as $branch)
                        <option value="{{ $branch->id }}"
                            {{ old('main_branch_fk', $record->main_branch_fk) == $branch->id ? 'selected' : '' }}>
                            {{ $branch->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select class="form-select form-control-3d" id="sub_branch_fk" name="sub_branch_fk" required>
                    <option value="">اختر الفرع الفرعي</option>
                    @foreach ($subBranches as $branch)
                        <option value="{{ $branch->id }}"
                            {{ old('sub_branch_fk', $record->sub_branch_fk) == $branch->id ? 'selected' : '' }}>
                            {{ $branch->title }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="number" class="form-control form-control-3d" id="sarf_rkm" name="sarf_rkm" required readonly
                    value="{{ old('sarf_rkm', $record->sarf_rkm) }}">
            </div>
            <div class="col-md-6 mb-3">
                <label for="sarf_to" class="form-label">الصرف إلى</label>
                <input type="number" class="form-control form-control-3d" id="sarf_to" name="sarf_to" required
                    value="{{ old('sarf_to', $record->sarf_to) }}">
            </div>
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="sanf_code" class="form-label">كود الصنف</label>
                <select class="form-select form-control-3d" id="sanf_code" name="sanf_code" required>
                    <option value="">اختر الصنف</option>
                    @foreach ($items as $item)
                        <option value="{{ $item->sanf_code }}"
                            {{ old('sanf_code', $record->sanf_code) == $item->sanf_code ? 'selected' : '' }}>
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="number" class="form-control form-control-3d" id="available_amount" name="available_amount"
                    required readonly value="{{ old('available_amount', $record->available_amount) }}">
            </div>
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="sanf_amount" class="form-label">كمية الصنف</label>
                <input type="number" class="form-control form-control-3d" id="sanf_amount" name="sanf_amount" required
                    value="{{ old('sanf_amount', $record->sanf_amount) }}">
            </div>
            <div class="col-md-6 mb-3">
                <label for="one_price_sell" class="form-label">سعر البيع للوحدة</label>
                <input type="number" class="form-control form-control-3d" id="one_price_sell" name="one_price_sell"
                    required value="{{ old('one_price_sell', $record->one_price_sell) }}">
            </div>
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
        ...
        <div class="row">
                ...
                <select class="form-select form-control-3d" id="publisher" name="publisher" required>
                    ...
                    @foreach ($publishers as $publisher)
                        <option value="{{ $publisher->id }}"
                            {{ old('publisher', $record->publisher) == $publisher->id ? 'selected' : '' }}>
                            {{ $publisher->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select class="form-select form-control-3d" id="publisher_name" name="publisher_name" required>
                    <option value="">اختر الناشر</option>
                    @foreach ($publishers as $publisher)
                        <option value="{{ $publisher->id }}"
                            {{ old('publisher_name', $record->publisher_name) == $publisher->id ? 'selected' : '' }}>
                            {{ $publisher->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <button type="submit" class="btn btn-primary">تحديث مصروف أصناف فرع</button>
