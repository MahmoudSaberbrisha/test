				...
				<div class="card mg-b-20">
					...
					<div class="card-body">
						<div class="table-responsive">
							<a href="javascript:void(0);" data-toggle="modal" data-target="#modal-add" class="btn btn-success mg-b-20">{{__('Add Booking')}} <i class="typcn typcn-plus"></i></a>
		...
		$(function() {
            table = $('#example1').DataTable({
                ...
                columns: [
					...
					},
					{
						data: null,
						render: function(data, type, row) {
							return row.sailing_boat && row.sailing_boat.name ? row.sailing_boat.name : '-';
						}
					},
            ...
            $('#example1 tbody').on('click', '.expand-btn', function () {
		        ...
		        if (row.child.isShown()) {
		            ...
		            $.ajax({
		                url: '{{ url(auth()->getDefaultDriver().'/booking-management/bookings/get-groups') }}/' + bookingId,
		                dataType: "JSON",
				        type: "GET",
				        data: [],
				        async: true,
		                success: function(response) {
		                    var groupHtml = '<table class="table table-sm table-bordered mt-2">' +
		                        '<thead>\
		                        	<tr>\
		                    ...
		                    if (response.length > 0) {
		                        response.forEach(function(group) {
		                            groupHtml += '<tr>' +
		                                ...
		                                '<td>' +  group.currency_symbol + ' ' + group.price + '</td>' +
		                                '<td>' +  group.currency_symbol + ' ' + group.discounted + '</td>' +
		                                '<td>' +  group.currency_symbol + ' ' + group.total + '</td>' +
		                                '<td>' +  group.currency_symbol + ' ' + group.paid + '</td>' +
		                                '<td>' +  group.currency_symbol + ' ' + group.remain + '</td>' +
		                                '<td>'+
										    '<label for="active' + group.id + '" class="switch">' +
										        '<input type="checkbox" id="active' + group.id + '" ' + (group.active == 1 ? 'checked' : '') +
										        ' onchange="changeSwitch(this, \'' + group.id + '\', \'' + '{{ route(auth()->getDefaultDriver().'.booking-group-active') }}' + '\');">' +
		                                ...
		                                '<td>' +
		                                	'<a class="remove-from-cart modal-effect" href="javascript:void(0);" ' +
												...
												'data-url="' + '{{ route(auth()->getDefaultDriver().'.booking-groups.destroy', '__ID__') }}'.replace('__ID__', group.id) + '" ' +
												'onclick="$(\'#modal-danger form\').attr(\'method\', \'post\'); $(\'#modal-danger form\').attr(\'action\', \'' + '{{ route(auth()->getDefaultDriver().'.booking-groups.destroy', '__ID__') }}'.replace('__ID__', group.id) + '\');"' +
												'data-toggle="modal" data-target="#modal-danger">' +
												'<i class="fa fa-trash"></i>' +
											'</a>\
											<a class="btn-edit" ' +
											    'href="' + '{{ route(auth()->getDefaultDriver().'.booking-groups.edit', '__ID__') }}'.replace('__ID__', group.id) + '" ' +
											    'data-placement="top" title="{{ __('Edit') }}">' +
											    '<i class="fa fa-edit"></i>' +
											'</a>\
											<a class="btn-sm-dark" target="_blank"' +
											    'href="' + '{{ route(auth()->getDefaultDriver().'.print-invoice', '__ID__') }}'.replace('__ID__', group.id) + '" ' +
											    'data-placement="top" title="{{ __('Print Invoice') }}">' +
											    '<i class="fa fa-print"></i>' +
											'</a>\
											<a class="btn-sm-info"' +
											    'href="' + '{{ route(auth()->getDefaultDriver().'.booking-group-extra-services', '__ID__') }}'.replace('__ID__', group.id) + '" ' +
		                        ...
		                        groupHtml += '<tr><td colspan="9" class="text-center">{{__('There are no groups.')}}</td></tr>';
		                    }
