                                ...
                                <div class="main-card-signin d-md-flex bg-white">
                                    <div class="wd-100p">
                                        <div class="main-signin-header">
                                            ...
                                            <form method="POST" action="{{ route('admin.password.email') }}" autocomplete="off" data-parsley-validate>
                                                @csrf
                                                <div class="form-group">
                                                    <label>{{__('Email')}}</label> 
                                                    <input name="email" value="{{old('email')}}" class="form-control" placeholder="{{__('Enter your email')}}" type="email" required>
                                                    @foreach ((array) $errors->get('email') as $message)
                                                        <p class="text-danger" id="throttle">{!! __($message) !!}</p>
                                                    @endforeach
                                                </div>
                                                <button type="submit" class="btn btn-main-primary btn-block">{{__('Send')}}</button>
                                            </form>
                                        </div>
